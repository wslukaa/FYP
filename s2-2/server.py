...
predictionAlgo = {'kr': ['Kernel Ridge Regression', 4.449910311], 'dt':['Decision Tree Regression', 6.318584563], 'kn':['KNN Regression', 6.084973799], 'lp':['LP Regression', 6.192667041], 'rf':['Random Forest Ridge Regression', 6.128285151], 'gp':['Gaussian Process Regression', 344.3642791] , 'ls':['Linear Support Vector Regression', 4.459506737] }
...
@app.route('/isTrendPrediction/<algo_tag_name>')
def render_isTrend(algo_tag_name):
	list_of_samples = get_list_of_samples()

	algo = algo_tag_name[:2]
	algo_name = predictionAlgo[algo][0]
	algo_tpr = 0.5
	...
	return render_template('isTrendPrediction.html',**locals())


@app.route('/valuePrediction/<algo_tag_name>')
def render_value(algo_tag_name):
	list_of_samples = get_list_of_samples()

	algo = algo_tag_name[:2]
	algo_name = predictionAlgo[algo][0]
	algo_rmse = predictionAlgo[algo][1]
	result = random() * 10
	...
	return render_template('valuePrediction.html',**locals())
...
@app.route("/valuePrediction")
def predictValue():
	return render_value('kr_crypto-20180206-145600')

@app.route("/isTrendPrediction")
def predictTrend():
	return render_isTrend('kr_crypto-20180206-145600')
